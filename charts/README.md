...
| config.k8s_facade | object | `{"bookmark_interval":"1m","enabled":false,"poll_interval":"1s","retention":"1h","ring_size":4096}` | Kubernetes-compatible read facade (discovery + list + watch) served at /apis/hyperfleet.openshift.io/v1alpha1 for client-go consumers. |
| config.k8s_facade.enabled | bool | `false` | Enable the facade routes and watch cache |
| config.k8s_facade.poll_interval | string | `"1s"` | Watch-cache poll cadence (LISTEN/NOTIFY only shortens the wait) |
| config.k8s_facade.ring_size | int | `4096` | Recent events kept per kind for watch replay |
| config.k8s_facade.retention | string | `"1h"` | resource_events history retention (410 Gone floor) |
| config.k8s_facade.bookmark_interval | string | `"1m"` | BOOKMARK event cadence on watches requesting them |
